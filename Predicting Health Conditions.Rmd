...
First, we will tune the cost parameter. Instead of minimizing the accuracy while tuning hyperparameters, we will minimize the F1 score. Accuracy is a misleading metric for imbalanced classes. A better metric is the F1 score because it combines the 
...
rocplot(-fitted.vals.1, diabetes.linear.test$had_diabetes)
abline(0, 1)
```
...
rocplot(-fitted.vals.2, diabetes.nonlinear.test$had_diabetes)
abline(0, 1)
```
...
13.55 is decent, but overfits the minority class by 15%.
...
```{r}
plot(svm.cancer, cancer.train, age ~ hours_worked_weekly)
```

Hours worked weekly is a strong predictor. Strangely, people whose working hours were about 2 to 3 standard deviations above the mean, were not predicted to have cancer, no matter their age. 

```{r}
plot(svm.cancer, cancer.train, age ~ height)
```

```{r}
plot(svm.cancer, cancer.train, age ~ weight, symbolPalette = c("red", "green"))
```

```{r}
plot(svm.cancer, cancer.train, age ~ bmi, symbolPalette = c("red", "green"))
```

```{r}
plot(svm.cancer, cancer.train, age ~ days_used_alcohol_yearly)
```
...
rocplot(-fitted.vals.3, cancer.test$had_cancer)
abline(0, 1)
```

```{r}
rocplot(-fitted.vals.1, cancer.test$had_cancer, col="red")
rocplot(-fitted.vals.2, cancer.test$had_cancer, col="blue")
rocplot(-fitted.vals.3, cancer.test$had_cancer, col="green")

abline(0, 1)
```

When we plot all 3 ROC curves together, the model for predicting cancer is the closest to the top left corner. That suggests that its overall performance, considering trade-offs, is the best. Although this model overfit the minority class, the ROC curve demonstrates that it performs best on the test data.
